...
async def call_llm(api_root_url, model, temperature, top_p, messages) -> str:
...
async def run_chunk_agent(api_root_url, model, temperature, top_p, chunk_text: str, query: str, idx: int, total: int) -> str:
    ...
    output = await call_llm(api_root_url, model, temperature, top_p, messages)
...
async def run_central_agent(api_root_url, model, temperature, top_p, query, round_report, history_messages) -> dict:
    ...
    output = await call_llm(api_root_url, model, temperature, top_p, history_messages)
...
async def run_query_pipeline(api_root_url, model, temperature, top_p, chunks: list[str], query: str) -> str:
    current_query = query
    history_messages = []
    for round_idx in range(1, MAX_ROUNDS + 1):
        tasks = [
            run_chunk_agent(api_root_url, model, temperature, top_p, chunk, current_query, i + 1, len(chunks))
        ...
        # 跑 query agent
        result = await run_central_agent(api_root_url, model, temperature, top_p, current_query, round_report, history_messages)
...
async def parallel_async_get_pred_for_sample(api_root_url, sample, model, tokenizer, temperature, top_p):
    ...
    response, history_messages = await run_query_pipeline(api_root_url, model, temperature, top_p, chunks, query)
